...
contract MintPool is LPTokenWrapper, Ownable
{
    IFLOKI public floki;
    ...
    function getReward() public payable updateReward(msg.sender) checkStart chargeFee {
        ...
        if (reward > 0) {
            ...
            mintLockedFloki(msg.sender, reward);
    ...
    function getBonus() public payable checkStart chargeFee {
        ...
        if (userBonus > 0) {
            bonus[msg.sender] = 0;
            mintLockedFloki(msg.sender, userBonus);
    ...
    function mintLockedFloki(address beneficiary, uint256 amount) internal {
        floki.mintLockedToken(beneficiary,amount);
    }
